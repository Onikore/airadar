...
#
# **Коммиты на HF.** Лимит — 128 в час на репозиторий. Части выгружаются
# каталогом раз в `CHECKPOINT_EVERY` шардов, одним коммитом на кэш, поэтому весь
# прогон стоит около 20 коммитов. Если лимит всё же выбит (например, после
# нескольких перезапусков) — `upload=False` считает всё локально и выгружает
# только готовый кэш в конце, это 2 коммита.

# %%
print("чекпоинт раз в", prep_hf.CHECKPOINT_EVERY, "шардов")
