...
- Конвенция проекта сохраняется: каждый модуль имеет `selfcheck()`, работающий **без данных и без GPU**, запускаемый как `python -m airadar.bench.<модуль> --selfcheck`.
...
def selfcheck():
    ...
    # стык в 1.0 с задевает окна, начинающиеся в 0.50, 0.75 и 1.00 (край)
    assert list(m) == [True, True, False, False, True, True], list(m)
...
```python
    # синтетический скорер: логит = оценка SNR в окне. Кривая обязана
    # пересечь 0.5 около нуля дБ при пороге, поставленном на 0.
    class SnrScorer:
        hop_s, context_s = 0.25, 0.5
        ...
        def score(self, audio):
            ...
            out = np.empty(n, np.float32)
            for i in range(n):
                w = audio[i * hop:i * hop + ctx]
                out[i] = 10 * np.log10(np.mean(w ** 2) + 1e-12) + 20.0
            return out

    rng2 = np.random.default_rng(7)
    tgt = rng2.normal(0, 0.05, 16000 * 4).astype(np.float32)
    pool = [rng2.normal(0, 0.05, 16000 * 4).astype(np.float32) for _ in range(4)]
    curve = p_detect_curve(SnrScorer(), tgt, pool, on=-2.0, off=-3.0, n_rep=4, seed=3)
    assert curve[0] >= curve[-1], curve          # монотонно не растёт вниз по SNR
...
def selfcheck():
    ...
    # несвязанные куски: сигнал в противофазе даёт большой скачок
    c = -x[:2000]
