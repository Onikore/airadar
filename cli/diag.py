...
if len(adj) == 0 or len(ctl) == 0:
    # Шарды DADS не перемешаны по меткам: столбец label прочитан по всем 39
    # файлам кэша (см. logs/d0_contiguity.log) — шарды 0-2 сплошь no-drone,
    # шард 3 смешанный (1767 pos / 2857 neg), шарды 4-38 сплошь drone. Пустой
    # шард (как 0) — не баг скрипта, а факт раскладки данных, и его надо
    # напечатать, а не уронить процесс в IndexError на пустом np.quantile.
