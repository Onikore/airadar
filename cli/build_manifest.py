...
def _reconcile(ck):
    ...
    совпавшем составе частей И непустом чекпоинте. Так выглядит прерванный шард
    (Ctrl-C, кончившееся место, OOM, падение декодера): байты доходят до файла
    по ходу записи, а n_samples_total растёт только по завершении целого шарда.
    Хвост за n_samples_total по построению не адресован ни одной готовой частью
    — та, что писалась, в чекпоинт так и не попала, — поэтому обрезка до
    обещанного размера ничего учтённого не теряет. Обрывать на этом
    многодневный прогон было бы дороже самой находки.

    Условие «непустой чекпоинт» здесь несущее, а не оговорка. При want == 0
    обрезка до обещанного размера — это truncate(0), то есть стирание всего
    файла. Пустой чекпоинт при непустом clips.bin — не прерванный шард, а
    потерянный учёт целой прошлой сборки (например, оператор удалил parts/ и
    чекпоинт, а clips.bin оставил), и это фатальный случай, а не повод обнулять
    гигабайты «в порядке восстановления».
    ...
    if tail > 0 and want == 0:
        # Нижняя граница обрезки. Без неё «хвост прерванного шарда» при пустом
        # чекпоинте означал бы truncate(0) — стирание всего файла, возможно
        # гигабайтов, в порядке восстановления.
        bad.append(f"clips.bin не пуст ({have} байт), а чекпоинт не помнит о нём "
                   f"ничего (0 отсчётов) — это не хвост прерванного шарда, а "
                   f"потерянный учёт целой прошлой сборки")
    if bad:
        if tail > 0 and want > 0:
    ...
    if tail > 0:
        # want == 0 отсечён выше как фатальный. Утверждение оставлено намеренно:
        # если ту ветку когда-нибудь ослабят, здесь должно упасть, а не молча
        # обнулить файл.
        assert want > 0, "обрезка при пустом чекпоинте — это стирание всей сборки"
