...
# %%
!pip install -q pyarrow soundfile "huggingface_hub[hf_xet]"
...
# DADS (39). Итого 85 шардов, 17,9 ГБ.
#
# **Где уходит время.** Счёт шарда — около 4 с, загрузка — секунды или десятки
# секунд, то есть узкое место целиком в сети. Поэтому шарды скачиваются целиком
# (а не читаются row-группами по HTTP, как было: 47 мелких range-запросов на
# файл) и по `PREFETCH` штук наперёд, пока считается текущий. Замерено на
# холодном кэше: 3,5 МБ/с было, 25,3 МБ/с стало — в 7 раз быстрее.
#
...
# %%
print(f"предзагрузка {prep_hf.PREFETCH} шардов наперёд, "
      f"чекпоинт раз в {prep_hf.CHECKPOINT_EVERY}")
