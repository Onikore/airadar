...
def selfcheck():
    ...
    # края выборки должны попадать в ресэмплировку не реже середины.
    # Проверяется прямо на счётчике вхождений: при начале блока из [0, n-block]
    # индекс 0 входил бы ровно в один блок против block вариантов у середины,
    # то есть в block раз реже, и хвост записи систематически недовешивался бы.
    idx = _resample_idx(200, 20, 4000, np.random.default_rng(3))
    cnt = np.bincount(idx.ravel(), minlength=200) / idx.size
    edge = np.r_[cnt[:20], cnt[-20:]].mean()
    mid = cnt[20:-20].mean()
    assert 0.9 < edge / mid < 1.1, (edge, mid)

    print("ci selfcheck ok")


def _resample_idx(n, block, n_boot, rng):
    """Индексы ЦИРКУЛЯРНОГО блочного бутстрапа: nb блоков подряд, обрезка до n.

    Начала блоков берутся из всего [0, n), а не из [0, n-block]: при усечённом
    диапазоне индексы 0 и последние block-1 попадают в меньшее число блоков,
    чем внутренние, и статистика систематически недовешивает края записи. Для
    полевой записи, где самые трудные окна как раз в начале и в конце, это
    сдвигает всё бутстрап-распределение. Выход за конец заворачивается по `% n`
    (это и делает бутстрап циркулярным) — стандартный приём против ровно этого
    смещения; ценой служит один искусственный стык «конец-начало» на блок.
    """
