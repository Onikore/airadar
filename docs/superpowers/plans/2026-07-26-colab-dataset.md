...
1. **`audio.array` — не байты, а вложенные списки float64.** 1 725,1 млн
   значений на все 168 записей, в среднем 10,3 млн на запись (~82 МБ float64).
   Вызов `.to_pylist()` раздует это в питоновские float по 28 байт — 1,25 ГБ
   на одну запись и падение по памяти на T4. Читать только через Arrow:
   `col.values.values.to_numpy(zero_copy_only=False)`.
...
каждый из трёх микрофонов. Итого ~30 часов канало-аудио при 16 кГц.
...
# 30 часов лабораторной записи двух машин не перевесили 164k окон DADS.
CAP = {
...
# %%
# Свободное место: кэш ~9 ГБ плюс шарды источников в кэше huggingface_hub.
!df -h /content | tail -1
...
1. Открыть `notebooks/01_prep.ipynb` в Colab, добавить секрет `HF_TOKEN`,
   ...
   440-470 тысяч окон, `cache_hard` порядка 60-75 тысяч, каждая часть сплита
...
**Незакрытый риск.** Числа окон в Task 8 («440-470 тысяч», «60-75 тысяч») —
