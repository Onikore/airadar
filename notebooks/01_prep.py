...
# | | ожидается | из чего |
# |---|---|---|
# | `cache_dads` | 370–395 тыс. окон | DADS дрон 164 тыс. + DADS фон ~177 тыс. + DroneAudioSet ~42 тыс. |
# | доля дрона | 50–58% | кэп фона 16 против 1 у дрона это и выравнивает |
# | `cache_hard` | 45–55 тыс. окон | ESC-50 ~10 тыс. + UrbanSound8K ~33 тыс. + тишина стенда ~6 тыс. |
#
# Если доля дрона окажется около 90% — перевёрнут `CAP`, это уже случалось.
# Проверка ниже ловит это сама.
...
for cache in ("cache_dads", "cache_hard"):
    ...
    torn = [int(gid) for gid in np.unique(g) if len(np.unique(sp[g == gid])) > 1]
    assert not torn, (
        f"{len(torn)} групп разорвано между сплитами, например {torn[:5]}. "
        f"Значит ключ группы не различает записи, которые должен различать.")
    if cache == "cache_dads":
        share = y.mean()
        assert 0.45 < share < 0.62, (
            f"дрон занимает {share*100:.1f}% окон — ожидается 50-58%. "
            f"Около 90% означает перевёрнутый CAP для DADS.")
        print(f"  доля дрона: {share*100:.1f}%")
    if "cat" in m.files:
